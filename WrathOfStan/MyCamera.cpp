...
void Simplex::MyCamera::CalculateViewMatrix(void)
{
	...
	m_m4View = glm::lookAt(m_v3Position, m_v3Target, glm::normalize(m_v3Upward)); //position, target, upward
}
...
void MyCamera::RotateCamera(float xAngle, float yAngle)
{
	// get the angle of the x and y
	quaternion m_xAngleAxis = glm::angleAxis(glm::radians(yAngle), glm::normalize(m_v3Position - m_v3Above));
	quaternion m_yAngleAxis = glm::angleAxis(glm::radians(xAngle), m_v3Rightward);
	quaternion m_totalRotate;

	// check if the camera is going vertically up or down
	if ((m_v3Forward.y < -.999 && m_yAngleAxis.x > 0) || (m_v3Forward.y > 0.999 && m_yAngleAxis.x < 0))
		m_totalRotate = m_xAngleAxis;
	else
		m_totalRotate = m_xAngleAxis * m_yAngleAxis;

	// get the new target position
	m_v3Target = m_v3Position + ((m_v3Target - m_v3Position) * m_totalRotate);
